------------------------------------------
GNU general public licence - èeský pøeklad
------------------------------------------
...
vìt¹inu programového vybavení nadace Free Software Foundation a na jakýkoli
jiný program, jeho¾ autor se pøikloní k jejímu pou¾ívání. (Nìkteré dal¹í
...
slu¾bu, pokud chcete), ¾e obdr¾íte zdrojový kód anebo jej mù¾ete získat,
pokud chcete, ¾e mù¾ete tento software mìnit nebo jeho èásti pou¾ít v nových
...
zaruèit, ¾e pøíjemci rovnì¾ dostanou anebo mohou získat zdrojový kód. A
...
Kvùli ochranì ka¾dého autora i nás samotných chceme zaruèit, aby ka¾dý
chápal skuteènost, ¾e pro volné programové vybavení nejsou ¾ádné záruky.
Je-li programové vybavení nìkým jiným modifikováno a posláno dále, chceme,
aby pøíjemci vìdìli, ¾e to, co mají, není originál, tak¾e jakékoliv problémy
vnesené jinými se neodrazí na reputaci pùvodních autorù.

Koneènì, ka¾dý volný program je neustále ohro¾en softwareovými patenty.
Pøejeme si zamezit nebezpeèí, ¾e redistributoøi volného programu obdr¾í
samostatnì patentová osvìdèení a tím uèiní program vázaným. Abychom tomu
zamezili, deklarovali jsme, ¾e ka¾dý patent musí být buï vydán s tím, ¾e
umo¾òuje volné u¾ití, anebo nesmí být vydán vùbec.
...
obsahuje zmínku, umístìnou v nìm dr¾itelem autorských práv, o tom, ¾e dílo
mù¾e být ¹íøeno podle ustanovení Obecné veøejné licence GNU. V dal¹ím textu
znamená ,,Program'' ka¾dý takový program nebo dílo a ,,dílo zalo¾ené na
Programu'' znamená buï Program samotný anebo ka¾dé jiné dílo z nìj odvozené,
...
licencí; sahají mimo její rámec. Akt spu¹tìní programu není omezen a výstup
z Programu je pokryt pouze tehdy, jestli¾e obsah výstupu tvoøí dílo zalo¾ené
...
Posouzení platnosti pøedchozí vìty závisí na tom, co Program dìlá.
  ...
  1. Smíte kopírovat a ¹íøit doslovné kopie zdrojového kódu Programu tak,
     ...
     kopii viditelnì a nále¾itì zveøejníte zmínku o autorských právech
     ...
     Za fyzický akt pøenesení kopie mù¾ete ¾ádat poplatek a podle vlastního
  ...
  2. Mù¾ete modifikovat va¹i kopii èi kopie Programu anebo kterékoliv jeho
     ...
     za pøedpokladu, ¾e splníte v¹echny tyto podmínky:
          ...
          nebo jeho èásti odvozeno, mohlo být jako celek bezplatnì
          poskytnuto ka¾dé tøetí osobì v souladu s ustanoveními této
          licence.

          c) Pokud modifikovaný program pracuje tak, ¾e ète
          ...
          Program redistribuovat, a musí u¾ivateli sdìlit, jakým
          zpùsobem mù¾e nahlédnout do kopie této licence. (Výjimka: v
     ...
     z Programu a mohou být samy o sobì rozumnì pova¾ovány za nezávislá
     ...
     musí být roz¹iøování tohoto celku podøízeno ustanovení této licence
     ...
     vámi ani popírání va¹ich práv vùèi nìmu; skuteèným smyslem je výkon
     práva na øízení distribuce odvozených nebo kolektivních dìl zalo¾ených
     na Programu.

     Pouhé spojení jiného díla, je¾ není na Programu zalo¾eno, s Programem
     (anebo dílem zalo¾eným na Programu) na pamì»ovém nebo distribuèním
  ...
  3. Mù¾ete kopírovat a roz¹iøovat Program (nebo dílo na nìm zalo¾ené, viz
     ...
     a 2 vý¹e, pokud splníte nìkterou z následujících nále¾itostí:

          a) Doprovodíte jej zdrojovým kódem ve strojovì èitelné formì.
          Zdrojový kód musí být roz¹iøován podle ustanovení sekcí 1
          a 2, a to na médiu bì¾nì pou¾ívaném pro výmìnu programového
          vybavení; nebo

          b) Doprovodíte je písemnou nabídkou nejménì na tøi roky,
          podle ní¾ poskytnete jakékoli tøetí stranì, za poplatek
          nepøevy¹ující va¹e výdaje vynalo¾ené na fyzickou výrobu
          zdrojové distribuce, kompletní strojovì èitelnou kopii
          odpovídající zdrojovému kódu, jen¾ musí být ¹íøen podle
          ustanovení sekcí 1 a 2 na médiu bì¾nì pou¾ívaném pro výmìnu
          ...
          s takovou nabídkou, v souladu s polo¾kou b vý¹e.)

     Zdrojový kód k dílu je nejvhodnìj¹í formou díla z hlediska jeho
     pøípadných modifikací. Pro dílo ve spustitelném tvaru znamená úplný
     zdrojový kód ve¹kerý zdrojový kód pro v¹echny moduly, které obsahuje,
     ...
     výjimkou jsou v¹ak ty programové komponenty, které jsou normálnì ¹íøeny
     (buï ve zdrojové nebo binární formì) s hlavními souèástmi operaèního
     systému, na nìm¾ spustitelný program bì¾í (tj. s pøekladaèem, jádrem
     apod.). Tyto komponenty nemusí být ¹íøeny se zdrojovým kódem, pokud
  ...
  4. Nesmíte kopírovat, modifikovat, poskytovat sublicence anebo ¹íøit
     Program jiným zpùsobem ne¾ výslovnì uvedeným v této licenci. Jakýkoli
     jiný pokus o kopírování, modifikování, poskytnutí sublicence anebo
     ¹íøení Programu je neplatný a automaticky ukonèí va¹e práva daná touto
  ...
  5. Není va¹í povinností tuto licenci pøijmout, proto¾e jste ji nepodepsal.
     ...
     modifikování a ¹íøení Programu a dìl na nìm zalo¾ených.
  6. Poka¾dé, kdy¾ redistribuujete Program (nebo dílo zalo¾ené na Programu),
     ...
     a podmínkami. Nesmíte klást ¾ádné pøeká¾ky výkonu zde zaruèených
     pøíjemcových práv. Nejste odpovìdný za vymáhání dodr¾ování této licence
     tøetími stranami.
  7. Jsou-li vám z rozhodnutí soudu, obvinìním z poru¹ení patentu nebo z
     ...
     svým závazkùm vyplývajícím z této licence a jiným platným zùvazkùm,
     ...
     va¹ím pøièinìním získají pøímo nebo nepøímo jeho kopie, pak by jediný
     mo¾ný zpùsob jak vyhovìt zároveò patentovému osvìdèení i této licenci
     spoèíval v ukonèení distribuce Programu.

     Pokud by se za nìjakých specifických okolností jevila nìkterá èást
     ...
     se pova¾uje za smìrodatný za jiných okolností.

     Smyslem tohoto paragrafu není navádìt vás k poru¹ování patentù
     èi jiných ustanovení autorského práva, anebo tato ustanovení
     zpochybòovat; jediným jeho smyslem je ochrana integrity systému ¹íøení
     volného programového vybavení, které je podlo¾eno veøejnými licenèními
     ...
     pøeje ¹íøit programové vybavení pomocí nìjakého jiného systému a ¾ádný
  ...
  8. Pokud je ¹íøení èi pou¾ití Programu v nìkterých zemích omezeno buï
     patenty anebo autorsky chránìnými rozhraními, mù¾e dr¾itel pùvodních
     autorských práv, který svìøuje Program do pùsobnosti této licence,
     pøidat výslovnì omezení pro geografické ¹íøení, vyluèující takové zemì,
  ...
  9. Free Software Foundation mù¾e èas od èasu vydávat upravené nebo nové
     verze Obecné veøejné licence. Takové nové verze se budou svým duchem
 ...
 10. Pokud si pøejete zahrnout èásti Programu do jiných volných programù,
     ...
     èiníme výjimky ze zde uvedených ustanovení. Na¹e rozhodnutí bude vedeno
 ...
 11. VZHLEDEM K BEZPLATNÉMU POSKYTNUTÍ LICENCE K PROGRAMU SE NA PROGRAM
     ...
     VÝSLOVNÉ NEBO VYPLÝVAJÍCÍ, VÈETNÌ, ALE NIKOLI JEN, ZÁRUK PRODEJNOSTI
     A VHODNOSTI PRO URÈITÝ ÚÈEL. POKUD JDE O KVALITU A VÝKONNOST PROGRAMU,
     ...
     VRUB.
 12. V ®ÁDNÉM PØÍPADÌ, S VÝJIMKOU TOHO, KDY® TO VY®ADUJE PLATNÝ ZÁKON, ANEBO
     KDY® TO BYLO PÍSEMNÌ ODSOUHLASENO, VÁM NEBUDE ®ÁDNÝ Z DR®ITELÙ
     AUTORSKÝCH PRÁV ANI ®ÁDNÁ JINÁ STRANA, KTERÁ SMÍ MODIFIKOVAT ÈI ©ÍØIT
     PROGRAM V SOULADU S PØEDCHOZÍMI USTANOVENÍMI, ODPOVÌDNÝ ZA ©KODY,
     VÈETNÌ V©ECH OBECNÝCH, SPECIÁLNÍCH, NAHODILÝCH NEBO NÁSLEDNÝCH ©KOD
     VYPLÝVAJÍCÍCH Z U®ÍVÁNÍ ANEBO NESCHOPNOSTI U®ÍVAT PROGRAMU (VÈETNÌ ALE
     ...
     S JINÝMI PROGRAMY), A TO I V PØÍPADÌ, ®E TAKOVÝ DR®ITEL AUTORSKÝCH
     PRÁV NEBO JINÁ STRANA BYLI UPOZORNÌNI NA MO®NOST TAKOVÝCH ©KOD.